...
uses
  { delphi }
  Windows, Contnrs, SysUtils, Forms,
  { local }
  ParseTreeNode,
  ParseTreeNodeType,
  ParseError,
...
begin
  ...
  begin
    ...
    lbOldStyleCharEscape := (lc2 <> nil) and (Length(lc2.Sourcecode) = 1) and
      not (TParserUtils.CharIsAlpha(lc2.Sourcecode[1]));
  end
  else
    lc2 := nil;
...
begin
  ...
  if ( not (lc.TokenType in EXPR_TYPES)) and TParserUtils.StrIsAlphaNum(lc.SourceCode) and
