...
uses
  ParseTreeNodeType, Tokens, Nesting, ParserUtils;

function NewReturn: TSourceToken;
begin
  Result := TSourceToken.Create;
  Result.TokenType := ttReturn;
  Result.SourceCode := #13;
end;
...
begin
  ...
  Result.SourceCode := TParserUtils.StrRepeat(' ', piLength);
end;
...
begin
  ...
  if (Pos(#13, pcToken.SourceCode) <= 0) then
    exit;
...
begin
  ...
  case peStrictness of
    ...
    idAny:
      ...
      Result := TParserUtils.StrIsAlpha(pt.SourceCode);
    else
    begin
      Result := False;
      Assert(false);
...
function IsSymbolOperator(const pt: TSourceToken): boolean;
begin
  Result := (pt.TokenType in Operators) and (not TParserUtils.StrIsAlpha(pt.SourceCode));
end;
