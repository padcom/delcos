...
begin
  ...
  begin
    ...
    if AnsiSameText(TParserUtils.StrLeft(psSourceCode, liItemLen), PreProcessorSymbolData[leLoop]) and
      ( not TParserUtils.CharIsAlpha(psSourceCode[liItemLen + 1])) then
    begin
      peSymbolType := leLoop;
      break;
    end;
  ...
  psText := TParserUtils.StrRestOf(psSourceCode, Length(PreProcessorSymbolData[peSymbolType]) + 1);

  if psText <> '' then
  begin
    if TParserUtils.StrRight(psText, 1) = '}' then
      psText := TParserUtils.StrChopRight(psText, 1);
