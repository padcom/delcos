{ $Id: NGUITestRunner.pas,v 1.4 2006/07/19 02:45:55 judc Exp $ }
{: DUnit: An XTreme testing framework for Delphi programs.
   @author  The DUnit Group.
   @version $Revision: 1.4 $ 2001/03/08 uberto
}
(*
...
uses
  ...
  SysUtils, Classes, IniFiles, ToolWin, System.ComponentModel;
...
type
  ...
  private
    ...
    procedure MenuLooksInactive(ACanvas: TCanvas; ARect: TRect; Selected: Boolean;
      Title: string; TitlePosn: UINT; PtyOveridesGUI: boolean);
    procedure MenuLooksActive(ACanvas: TCanvas; ARect: TRect; Selected: Boolean;
      Title: string; TitlePosn: UINT);
...
//  (ScoreBar as TProgressBarCrack).Color := clOK;
//  (ScoreBar as TProgressBarCrack).RecreateWnd;
  ScoreBar.color := clOk;
end;
...
begin
  ...
  begin
    OverridesGUI :=
      ((FailIfNoChecksExecuted.Checked and not Test.FailsOnNoChecksExecuted));
    HasRunTimePropsSet :=
      ((Test.FailsOnNoChecksExecuted and not FailIfNoChecksExecuted.Checked));
...
begin
// ATTENTION
// when called first time after ShowModal the ResultView doesn't contain any items
...
begin
  ...
  item.data := (TestToNode(failure.failedTest));
...
procedure TGUITestRunner.WarnOnFailTestOverrideActionExecute(Sender: TObject);
begin
